...
## 2026-07-05 (심야 7) — v0.7.1 UI 고급 폴리시

정체성 유지 + 완성도 정교화. 컴포넌트가 이미 섀도 토큰을 참조 → 토큰 개선이 전역 반영(최소 변경·최대 효과).
- 섀도 토큰 2단 소프트 재정의(--shadow-sm/--shadow/--shadow-lg), 라운드 스케일·표면 틴트 토큰 추가
- 눌림 마이크로 인터랙션(카드·버튼·칩 :active scale 0.975 + transition), prefers-reduced-motion 존중
- 히어로 상단 내부 하이라이트로 프리미엄 sheen
- 브라우저 360/1440 오버플로우·콘솔 0, 브랜드·레이아웃 불변. 도전과제 컬렉션 화면이 폴리시 쇼케이스

배포: v0.7.1 태그 + push → Vercel.
