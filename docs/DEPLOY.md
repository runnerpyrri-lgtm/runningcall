# 야외봄 배포 가이드 (1단계: 컴퓨터 꺼져도 작동)

이 문서만 따라 하면 야외봄이 클라우드에서 24시간 작동합니다.
...
2. Repository name: `outbom` / 나머지 체크박스는 다 끄고 → **Create repository**
...
공식 운영 화면은 GitHub Pages `https://robom-labs.github.io/outbom/`입니다. 현재 Vercel 프로젝트 `runningcall`은 Pages가 사용하는 API와 기존 설치 사용자를 위해 `https://outbom.robom.kr`, `https://runningcall.vercel.app`에서 계속 제공합니다.
새 프로젝트를 만들지 말고 기존 프로젝트의 Git 연결과 배포 상태를 관리합니다.

1. https://vercel.com → **Continue with GitHub** (같은 계정으로 로그인)
2. 기존 **runningcall** 프로젝트 → **Settings → Git**으로 이동
3. 연결 저장소가 `robom-labs/outbom`인지 확인
4. `main` 브랜치에 변경이 병합되면 자동 배포가 완료될 때까지 대기
4. 1~2분 후 `https://robom-labs.github.io/outbom/`에서 새 운영 화면을 확인하고 `https://outbom.robom.kr/api/forecast` API도 함께 확인합니다.
5. 폰에서 공식 주소로 접속 → Chrome 메뉴 → **앱 설치** 로 PWA 설치 테스트
...
- 공식 도메인은 Vercel에 연결되어 있으며 기존 Vercel 주소는 호환용으로 유지합니다.
