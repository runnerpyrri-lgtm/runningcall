...
## Task 5: 등산 데이파트 — 4구간 유지 확인 (변경 없음, 회귀 방지 테스트만)

**결정 변경**: 사용자 재검토 결과 저녁 구간 제거를 철회. 새벽(일출 전 출발)이 있는데 저녁(하산 시간대·노을 산행·짧은 저녁 코스)이 없는 게 비대칭이라는 판단. "산행 위험" 라벨은 저녁 전용 하드코딩이 아니라 다른 활동과 동일한 공통 `blockedTag` 로직(그날 그 구간에 조건 맞는 점수가 없을 때만 표시)이므로 저녁 구간 자체를 없앨 이유가 아님. **HIKE_DAY_PARTS는 새벽/아침/낮/저녁 4구간 그대로 유지.**

**Files:** `lib/insights.ts` (변경 없음 확인), `lib/__tests__/scoring.test.ts` (회귀 방지 테스트만 추가)

- [ ] **Step 1: 현행 4구간 유지 확인**
  `grep -n "HIKE_DAY_PARTS" -A 6 lib/insights.ts`로 새벽/아침/낮/저녁 4개 항목이 그대로인지 확인. 수정하지 않음.

- [ ] **Step 2: 4구간 유지 회귀 테스트 추가**

`lib/__tests__/scoring.test.ts`에 추가(기존 "새벽 포함 4구간" 테스트가 있다면 그대로 두고, 저녁 포함 여부를 명시적으로 검증하는 테스트를 보강):
```ts
  it("등산 데이파트는 새벽·아침·낮·저녁 4구간을 유지한다 (비대칭 방지)", async () => {
  ...
  it("저녁 구간의 차단 여부는 다른 구간과 동일한 기준(blockedTag)을 쓴다", async () => {
    // 저녁이라고 항상 막히는 하드코딩이 없는지 — best가 있으면 past/blocked가 아니어야 한다
    const { getDayParts } = await import("@/lib/insights");
    const parts = getDayParts([], false, 6, "hike");
    const evening = parts.find((p) => p.label === "저녁");
    expect(evening).toBeDefined();
    // 슬롯이 없는 입력이므로 best는 null이 정상 — "항상 위험"이 아니라 "데이터가 없어서 없음"임을 확인
...
- [ ] **Step 3: 검증 + Commit**
  ...
  git commit -m "test: 등산 4구간(새벽/아침/낮/저녁) 유지 회귀 테스트 — 저녁 제거 결정 철회

  새벽만 있고 저녁이 없는 비대칭을 피하기 위해 4구간 유지로 재결정.
  '산행 위험'은 저녁 전용이 아니라 공통 blockedTag 로직임을 테스트로 고정.
...
- [ ] **Step 3: 문서**
  ...
  - `CHANGELOG.md` `[0.4.2]`: 운동 일지, 동적 가이드, 애견 플랜 심화, 좌측 레일 정리, 등산 4구간 유지 확정, 활동별 위치, 죽은 코드 제거.
...
## Self-Review
- [ ] 스펙 9개 결정 대응: 칩(T2), 애견심화(T3), 일지(T4·T8), 등산4구간유지+회귀테스트(T5), 동적가이드(T6), 레일+tagline제거+위치(T7), 죽은코드(T1).
