...
const BRIEFING_VERDICT: Record<ActivityKey, Array<[number, string[]]>> = {
  ...
  hike: [
    [88, ["완벽한 산행 날씨예요", "오늘 산 안 가면 아까워요", "산행 최고의 날이에요", "조망까지 완벽한 날이에요"]],
    [72, ["산행하기 좋은 날이에요", "가볍게 오르기 딱 좋아요", "기분 좋은 산행이 될 거예요", "부담 없이 다녀오기 좋아요"]],
    [55, ["무난한 편이에요", "가벼운 코스는 충분해요", "컨디션 보며 올라봐요", "낮은 산이라면 괜찮아요"]],
    [38, ["짧은 코스만 추천해요", "정상 욕심은 접어둬요", "무리는 아쉬워요", "둘레길이 나아요"]],
    [0, ["오늘 산행은 위험해요", "쉬어가도 괜찮아요", "무리할 날은 아니에요", "안전이 우선이에요"]]
  ],
  bike: [
...
export function composeBriefing(reference: RunningSlot, best: RunningSlot, isTomorrow: boolean, activity: ActivityKey = "run"): Briefing {
  ...
  const caution = (() => {
    ...
    switch (weakest.key) {
      case "precip":
        return isRun ? "비 예보, 짧은 코스로" : isHike ? "비 예보, 미끄럼·하산 주의" : "비 예보, 우산 챙기기";
      case "dust":
        return isDog ? "미세먼지, 산책 짧게" : isRun ? "미세먼지, 강도 낮게" : isHike ? "미세먼지, 조망 아쉬움" : "미세먼지, 마스크 챙기기";
      case "temp":
      ...
      case "wind":
        return isRun ? "바람 강함, 맞바람 먼저" : activity === "bike" ? "강풍 주의, 안전 우선" : isHike ? "능선 강풍, 정상 주의" : "바람 강함, 겉옷 챙기기";
      case "humidity":
  ...
  const tip = (() => {
    ...
    if (reference.windSpeed >= 8) return isRun ? "바람막이 한 겹" : activity === "bike" ? "강풍 구간 감속" : isHike ? "능선에서 방풍 필수" : "겉옷 한 겹 챙기기";
    const generic: Record<ActivityKey, string[]> = {
      ...
      hike: ["하산 시간 미리 계산", "물·간식 넉넉히", "무릎 보호 스틱", "정상 방한 한 겹", "예보 다시 확인", "헤드랜턴 챙기기"],
        ...
        meaning:
          amount >= 1
            ? isRun
              ...
              : isBike
              ? "비가 내려요. 노면이 미끄럽고 제동거리가 길어져 위험해요."
              : isHike
              ? "비가 내려요. 바위·계단이 미끄러워 하산이 특히 위험해요."
